...
export default function VIPNetwork() {
  ...
  const getListParent = async (limit, page) => {
    setLoading(true);
    try {
      let response = await axiosService.post("api/user/getParentList", { userid: uid, limit, page });
  ...
  const handleSearch = async () => {
    if (searchBy == "byLevel") {
      setCurrent(1);
      await searchListParent_byLevel(uid, ROW_PER_TABLE, 1, levelValue);
    } else if (searchBy == "byUsername") {
      setCurrent(1);
      await searchListParent_byName(uid, ROW_PER_TABLE, 1, userNameValue);
    }
  };
          ...
          pagination={{
            ...
            onChange: async (page) => {
              ...
              if (mode == 1) {
                ...
                await searchListParent_byName(uid, ROW_PER_TABLE, page, userNameValue);
              } else if (mode == 3) {
                await searchListParent_byLevel(uid, ROW_PER_TABLE, page, levelValue);
              }
            },
          }}
        />
